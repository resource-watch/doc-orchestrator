...
describe('STATUS_FINISHED_REINDEX handling process', () => {
    ...
    it('Consume a STATUS_FINISHED_REINDEX message for a TASK_CONCAT of a dataset with no sources or connectorUrl should create a EXECUTION_CONFIRM_REINDEX message and patch the dataset without connectorUrl or sources', async () => {
        const taskData = createTask({
            status: appConstants.TASK_STATUS.INIT,
            type: task.MESSAGE_TYPES.TASK_CONCAT,
        });
        taskData.message.fileUrl = [];
        const fakeTask1 = await new Task(taskData).save();
